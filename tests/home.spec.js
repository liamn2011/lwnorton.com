...
test("has hero content", async ({ page }) => {
  ...
  await expect(page.locator("#gatsby-focus-wrapper")).toContainText(
    "a Software Engineer building impactful solutions. Let's delve into the realm where technology meets efficiency, where my passion for innovation thrives."
  );
  await expect(
    page.getByRole("img", { name: "svg of Liam Norton" })
  ).toBeVisible();
});
