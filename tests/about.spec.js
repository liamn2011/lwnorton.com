...
test("click About Link, check content", async ({ page }) => {
	...
	await expect(page.getByRole("main")).toContainText(
		"Hey, I'm Liam, a Manchester based technical professional driven by a passion for coding and a commitment to building innovative solutions. With a track record of delivering impactful projects, I thrive at the intersection of technology and problem-solving. Staying at the forefront of evolving advancements, I bring a dynamic approach to addressing challenges and driving positive change."
	);
	await expect(page.getByRole("main").getByRole("img", { name: "Liam Norton" })).toBeVisible();
	await expect(page.getByRole("main")).toContainText("PROFESSIONAL SKILLS");
	await expect(page.getByRole("main")).toContainText("SKILLS");
	await expect(page.getByText("SKILLSSolution Design95%Web")).toBeVisible();
	await expect(page.getByText("EXPERIENCEJan 2023 to")).toBeVisible();
	await expect(page.locator("div").filter({ hasText: "ABOUT MEDownload CVHey, I'm" }).nth(3)).toBeVisible();
	await expect(page.getByText("PROFESSIONAL SKILLSSKILLSSolution Design95%Web Development93%System Analysis93%")).toBeVisible();
	await expect(page.getByText("ABOUT MEDownload CV")).toBeVisible();
	await expect(page.locator("section").filter({ hasText: "Hey, I'm Liam, a Manchester" })).toBeVisible();
